<?php
require_once("Models/home.php");
class HomeController
{
    var $home_model;
    public function __construct()
    {
       $this->home_model = new Home();
    }
    
    function list()
    {
        $data_danhmuc = $this->home_model->danhmuc();

        $data_chitietDM = array();

        for($i=1; $i <=count($data_danhmuc);$i++){
            $data_chitietDM[$i] = $this->home_model->chitietdanhmuc($i);
        }

        $data_limit1 = $this->home_model->limit(0,10);
        $data_limit2 = $this->home_model->limit(4,4);
        $data_random = $this->home_model->random(8);

        $data_banner = $this->home_model->banner();

        $data_sanpham1 = $this->home_model->sanpham_danhmuc(0,8,1);
        $data_sanpham2 = $this->home_model->sanpham_danhmuc(0,8,2);
        $data_sanpham3 = $this->home_model->sanpham_danhmuc(0,8,3);
        $data_sanpham4 = $this->home_model->sanpham_danhmuc(0,8,4);
        $data_tuychon1 = $this->home_model->sanpham_tuychon(0,10,4);
        $data_tuychon2 = $this->home_model->sanpham_tuychon(0,10,6);
        $data_tuychon3 = $this->home_model->sanpham_tuychon(0,10,1);
        $data_tuychon4 = $this->home_model->sanpham_tuychon(0,10,2);
        $data_tuychon5 = $this->home_model->sanpham_tuychon(0,10,3);
        
        $thuonghieu = $this->home_model->thuonghieu();
        require_once('Views/index.php');
    }
}